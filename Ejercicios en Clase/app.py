...
@app.route('/')
def index():
    # Create an object of the PVModel class
    pv = PVModel(4, 3)
    ...
    pv_pv_curve_buf = BytesIO()
    pv_pv_curve.savefig(pv_pv_curve_buf, format='png')
    pv_pv_curve_b64 = 'data:image/png;base64,' + pv_pv_curve_buf.getvalue().decode('utf-8')
    pv_iv_curve_buf = BytesIO()
    pv_iv_curve.savefig(pv_iv_curve_buf, format='png')
    pv_iv_curve_b64 = 'data:image/png;base64,' + pv_iv_curve_buf.getvalue().decode('utf-8')
    # Render the HTML template
    return render_template('index.html', pv_pv_curve_b64=pv_pv_curve_b64, pv_iv_curve_b64=pv_iv_curve_b64)
